...
//
function train() {
    let trainTxt = document.getElementById("trainTxt").value;
    console.log(`Train ${trainTxt}`);
    knn.learn(data.split`,`.map(x=>+x), trainTxt)
}
...
//
function classify() {
    let classify = knn.classify(data.split`,`.map(x=>+x))
    console.log(`I think this is: ${classify}`);
    predictionTxt.innerHTML = `I think this is: ${classify}`;
}
